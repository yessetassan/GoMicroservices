CREATE TABLE IF NOT EXISTS role (
                                    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                    code VARCHAR(50) NOT NULL,
                                    deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS _user (
                                     id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                     first_name VARCHAR(50) NOT NULL,
                                     last_name VARCHAR(50) NOT NULL,
                                     middle_name VARCHAR(50),
                                     login VARCHAR(30) NOT NULL UNIQUE,
                                     email VARCHAR(50) NOT NULL UNIQUE,
                                     password_hash VARCHAR NOT NULL,
                                     role_id BIGINT REFERENCES role(id)
);

CREATE TABLE IF NOT EXISTS category (
                                        id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                        category_code VARCHAR(50) NOT NULL,
                                        description TEXT,
                                        deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS inventory (
                                         id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                         quantity INT NOT NULL,
                                         description TEXT,
                                         deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS product (
                                       id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                       product_name VARCHAR(100) NOT NULL,
                                       category_id BIGINT REFERENCES category(id),
                                       inventory_id BIGINT REFERENCES inventory(id),
                                       price DOUBLE PRECISION NOT NULL CHECK (price > 0),
                                       description TEXT NOT NULL,
                                       deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS user_payment (
                                            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                            user_id BIGINT REFERENCES _user(id),
                                            payment_type VARCHAR(255),
                                            provider VARCHAR(255),
                                            account_no VARCHAR(255),
                                            expiry DATE NOT NULL,
                                            deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS payment_status (
                                              id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                              name VARCHAR(255),
                                              deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS cart_item (
                                         id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                         user_id BIGINT REFERENCES _user(id),
                                         product_id BIGINT REFERENCES product(id),
                                         quantity INT,
                                         total_price DOUBLE PRECISION NOT NULL CHECK (total_price >= 0)
);

CREATE TABLE IF NOT EXISTS _order (
                                      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                      user_id BIGINT REFERENCES _user(id),
                                      product_id BIGINT REFERENCES product(id),
                                      quantity INT,
                                      total_price DOUBLE PRECISION NOT NULL CHECK (total_price > 0),
                                      deleted BOOLEAN DEFAULT FALSE
);

CREATE TABLE IF NOT EXISTS payment_details (
                                               id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                               order_id BIGINT REFERENCES _order(id),
                                               payment_id BIGINT REFERENCES user_payment(id),
                                               status_id BIGINT REFERENCES payment_status(id),
                                               deleted BOOLEAN DEFAULT FALSE
);
INSERT INTO role (code) VALUES
                            ('USER'),
                            ('ADMIN');


INSERT INTO category (category_code, description) VALUES
                                                      ('ELEC', 'Electronics'),
                                                      ('CLOTH', 'Clothing'),
                                                      ('FOOD', 'Food Items'),
                                                      ('BOOK', 'Books'),
                                                      ('FURN', 'Furniture'),
                                                      ('SPORT', 'Sports Equipment'),
                                                      ('TOY', 'Toys'),
                                                      ('BEAUT', 'Beauty Products'),
                                                      ('PHARM', 'Pharmaceuticals'),
                                                      ('GARD', 'Gardening Supplies');


INSERT INTO inventory (quantity, description) VALUES
                                                  (100, 'Laptops and Tablets'),
                                                  (200, 'Men and Women Clothing'),
                                                  (300, 'Packaged Foods'),
                                                  (150, 'Educational and Fiction Books'),
                                                  (50, 'Home and Office Furniture'),
                                                  (120, 'Indoor and Outdoor Sports Gear'),
                                                  (140, 'Children Toys'),
                                                  (80, 'Skincare and Makeup'),
                                                  (60, 'Medicines and Health Products'),
                                                  (90, 'Plants and Gardening Tools');
INSERT INTO payment_status (name) VALUES
                                      ('Pending'),
                                      ('Completed'),
                                      ('Failed'),
                                      ('Refunded');
